    ...
    static void Main(string[] args)
    {
        using (StreamReader reader = File.OpenText(args[0]))
            while (!reader.EndOfStream)
            {
                ...
                Dictionary<double, int> histogram = samples.GroupBy(c => c).ToDictionary(g => g.Key, g => g.Count());

                List<AForge.Math.Complex> samplesComplex = new List<AForge.Math.Complex>();
                ...
                double valueMax = samples.Max();
                double valueMin = samples.Min();
                ...
                //foreach(double d in samples)
                {
                    ...
                    {
                        AForge.Math.Complex c = new AForge.Math.Complex(normalized[i], 0);
                        samplesComplex.Add(c);
                    }
                    else
                    {
                        AForge.Math.Complex c = new AForge.Math.Complex(0, 0);
                        samplesComplex.Add(c);
                    }
                }
                ...
                AForge.Math.Complex[] data = samplesComplex.ToArray();
                ...
                foreach(AForge.Math.Complex com in data)
                {
                    result.Add(Math.Sqrt((com.Re * com.Re) + (com.Im * com.Im)));
                }
                
                double res = result.Max();
                int removed = 0;
                while (result.IndexOf(res) + removed > 200 || result.IndexOf(res) + removed < 15)
                {
                    result.Remove(res);
                    removed++;
                    res = result.Max();
                ...
                double multiple = (20000d / (double)sampleLength);
                
                double freq = Math.Round(((result.IndexOf(res) + Math.Floor((double)removed / 2)) * multiple)/10)*10;
